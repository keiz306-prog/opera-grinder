...
st.caption("v3.1 Update - 순정 더블 비가압 실측 프로파일 (피크 11.5 bar ➔ 종료 10.0 bar) 정밀 반영")
...
# 1번 탭: 동적 추출 예측기 (오페라)
with tab1:
    st.subheader("동적 도징 & 압력/유속 예측 대시보드 (v3.1 - 더블 실측 11.5➔10.0bar 동기화)")
    ...
    # --- 더블 바스켓 전용 도징 연산 (20.0 클릭 ➔ 17.8g) ---
    base_calc_double = bean_info['base_dose'] + (target_dial - bean_info['base_dial']) * 0.644
    calculated_dose_double = round(max(1.0, base_calc_double - (grind_diff * 0.5)), 1)

    # --- 싱글 바스켓 전용 도징 연산 (10.5 클릭: 11.0g / 11.0 클릭: 12.0g) ---
    if target_dial >= 11.0:
        base_calc_single = 12.0 + (target_dial - 11.0) * 0.644
    elif target_dial >= 10.5:
        base_calc_single = 11.0 + (target_dial - 10.5) * 2.0
    else:
        base_calc_single = max(1.0, 11.0 - (10.5 - target_dial) * 1.0)
    calculated_dose_single = round(max(0.5, base_calc_single - (grind_diff * 0.5)), 1)

    with col_a2:
        st.markdown("**그라인더 예측 토출량 (2샷 더블 모드)**")
    ...
    # 1. 순정 더블 비가압 (1단 / 20.0클릭 / 17.8g 수동 모드 ➔ 피크 11.5 bar / 종료 10.0 bar)
    ...
    estimated_end_double = round(max(2.0, estimated_peak_double - 1.5), 1)  # 추출 후반부 1.5 bar 감쇄
    
    flow_double = round(3.2 * (bean_info['base_dose'] / max(calculated_dose_double, 5.0)), 1)

    # 2. 순정 싱글 비가압
    single_base_pressure = 10.5 + (calculated_dose_single - 11.0) * 1.0 - (grind_diff * 1.3)
    if "자동" in extraction_mode:
        single_base_pressure -= 1.5
    estimated_peak_single = round(max(2.0, min(15.0, single_base_pressure)), 1)
    
    flow_single = round(max(0.3, 1.2 - (calculated_dose_single - 11.0) * 0.4), 2)
    ...
    if estimated_peak_double >= 13.0:
        ...
        st.info(f"🟡 **[표준 / 준고압 구간 (피크 {estimated_peak_double} bar ➔ 종료 {estimated_peak_double - 1.5:.1f} bar)]**: 실측 적정 저항 세팅입니다.")
    else:
        st.success(f"🟢 **[저압 추출 구간 ({estimated_peak_double} bar)]**: 빠른 흐름 영역입니다.")
...
# 4번 탭: 2D 도징 계산 모델 설명
with tab4:
    st.subheader("📐 2D 도징 계산 모델 설명 (v3.1 더블 실측 11.5➔10.0 bar 반영)")
    st.markdown("""
    - **순정 더블 비가압 실측 앵커 고정:**
      - **분쇄도 1단 / 다이얼 20.0:** 17.8g / 피크 압력 **11.5 bar** ➔ 종료 시 **10.0 bar**로 감쇄
    - **순정 싱글 비가압 실측 앵커 고정:**
